...
class Url {
	...
	*/
	...
	static function humanize(string $input, $minDomainPartLength = 8): string {

		$pass = preg_match('~^\s*([a-z]{3,8}://)?([^\/\?]+)\/?(.*)\s*$~i', $input, $matches);

		if ($pass) {
			if ($matches[2] && strlen($matches[2]) >= $minDomainPartLength) {
				return $matches[2];
			}
	...
	static function technicalize(string $input, $defaultScheme = 'https') {

		$pass = preg_match('~^\s*(\w{3,15}:\/\/).*$~', $input, $matches);
